...
class GCORE(
  ...
  private val nodeEdgeVars: (Set[Var], Set[Var]) =
    pattern.fullGraphPattern.union(template.fullGraphPattern).foldLeft((Set(), Set()))((acc, b) => b match
  ...
  val leftNodeVariables: Set[Var] =
    pattern.fullGraphPattern.union(template.fullGraphPattern).flatMap(b => b match
    ...
    Util.sequence(fgp.toList.map {
      case BasicGraphPattern.NodePattern(x) =>
        ...
        if labels.isEmpty && kvs.isEmpty && keys.isEmpty && !vars.contains(x.toVar) then None
          ...
          Some(
            labels.map { l =>
              AtomicPattern.VAC(vx, l.toIri(node = nodeVariables.contains(vx)))
            }.concat(
            kvs.map { (k,v) =>
  ...
  private def removeToFilter(r: RemoveClause): FilterPattern =
    r match
      case RemoveClause.RemoveKey(x, k) => FilterPattern.notP(x.toVar, k.toIri(node = nodeVariables.contains(x.toVar)))
      case RemoveClause.RemoveLabel(x, l) => FilterPattern.notC(x.toVar, l.toIri(nodeVariables.contains(x.toVar)))
  ...
  val toSCCQ: Option[SCCQ] =
    for h <- sccqTemplate
        p <- sccqPattern
    yield SCCQ(template = h, pattern = p, eccq = Some(ECCQ(
...
object GCORE:
  ...
  case class Key(keyname: String) extends Showable:
    ...
    def toIri(node: Boolean): Iri =
      Iri.fromString(s"<${if node then Key.nodeIri else Key.edgeIri}/${keyname}>").toOption.get
  ...
  case class Label(labelname: String) extends Showable:
    ...
    def toIri(node: Boolean): Iri =
      Iri.fromString(s"<${if node then Label.nodeIri else Label.edgeIri}/${labelname}>").toOption.get
  ...
  object Label:
    ...
    def fromIri(i: Iri, node: Boolean): Label =
      val base = Iri.fromString(s"<${if node then Label.nodeIri else Label.edgeIri}/>").toOption.get
  ...
  val nodeToEdgeIri = Iri.fromString(s"<https://github.com/softlang/s2s/meta_nte>").toOption.get

  /** Internal role for 'out' edges. */
  val nodeToEdgeRole: NamedRole = NamedRole(nodeToEdgeIri)

  /** Internal IRI for 'in' edges. */
  val edgeToNodeIri = Iri.fromString(s"<https://github.com/softlang/s2s/meta_etn>").toOption.get

  /** Internal role for 'in' edges. */
  val edgeToNodeRole: NamedRole = NamedRole(edgeToNodeIri)

  /** Internal IRI for 'node'. */
  val node = Iri.fromString(s"<https://github.com/softlang/s2s/meta_node>").toOption.get

  /** Internal Concept for 'node'. */
  val nodeConcept: NamedConcept = NamedConcept(node)

  /** Internal IRI for 'edge'. */
  val edge = Iri.fromString(s"<https://github.com/softlang/s2s/meta_edge>").toOption.get
  ...
  def shapeToSetClause(shape: SHACLShape)(implicit scopes: Scopes): S2STry[SetClause] =
    ...
    shape.axiom.c match
      case NamedConcept(ci) if ci.isVariable =>
        ...
        shape.axiom.d match
          case NamedConcept(d) =>
            Right(SetClause.SetLabel(v, Label.fromIri(d, node = true))) // TODO: What to select here?
          case Existential(NamedRole(r), NominalConcept(i)) =>
            Right(SetClause.SetKeyValue(v, Key.fromIri(r, node = true), Value.fromIri(i))) // TODO: ditto
          case _ =>
