...
enum ShapeHeuristic(val optimize: Boolean):
  ...
  case SimpleShapes(
      ...
      proxyFamily: Boolean = true
  ) extends ShapeHeuristic(optimize = opt)
  ...
  // New variant for generation of random ProGS samples.
  case NovaProGS(depth: Int, breadth: Int, opt: Boolean)
      extends ShapeHeuristic(optimize = opt)
