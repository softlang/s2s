...
enum AlgorithmInput:
  ...
  val template: S2STry[AtomicPatterns] = this match
    ...
    case GCOREAxioms(q, _) => q.toSCCQ.map(_.template) match
      case None => Left(
  ...
  val pattern: S2STry[AtomicPatterns] = this match
    ...
    case GCOREAxioms(q, _) => q.toSCCQ.map(_.pattern) match
      case None => Left(
        UnsupportedQueryError(q, details = "This GCORE query can not be converted to SPARQL.")
      )
      case Some(p) =>
  ...
  def formatShapes: S2STry[String] = this match
    case SCCQSimpleSHACL(_, shacl, _) => JsonLDParser.unparse(shacl.map(_.dropScopeS))
