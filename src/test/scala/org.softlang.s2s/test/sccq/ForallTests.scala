...
class ForallTests extends ValidationSuite:
  ...
  includes("general_0_0", noshapes, q0, noshapes)

  includes("general_0_1", Set(":A ⊑ ∀:p.:A"), q0, Set(":B ⊑ :C", ":C ⊑ :B"))

  includes("general_0_2", Set(":B ⊑ :A", ":A ⊑ ∀:p.:B"), q0, Set(":B ⊑ :C", ":C ⊑ :B"))

  val q1 = query("?y a :C . ?z a :D", "?x a :A . ?x :p ?y . ?z a :B")

  includes("general_1_0", Set(":A ⊑ :B", ":A ⊑ ∀:p.:A"), q1, Set(":C ⊑ :D"))
