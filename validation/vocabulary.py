...
class Vocabulary:
    ...
    meta_etn_raw = "https://github.com/softlang/s2s/meta_etn"
    meta_nte_raw = "https://github.com/softlang/s2s/meta_nte"
    meta_etn = URIRef(meta_etn_raw)
    meta_nte = URIRef(meta_nte_raw)

    meta_node_raw = "https://github.com/softlang/s2s/meta_node"
    meta_edge_raw = "https://github.com/softlang/s2s/meta_edge"
    meta_node = URIRef(meta_node_raw)
    meta_edge = URIRef(meta_edge_raw)

    otherval_raw = "https://github.com/softlang/s2s/gentype-sumin"
    otherval = URIRef(otherval_raw)

    epp_raw = "https://github.com/softlang/s2s/ekey"
    npp_raw = "https://github.com/softlang/s2s/nkey"
    ...
    def __init__(self, concepts, properties, nominals):
        """Initialize a vocabulary from its components, parsed from files."""
        # Load the raw data from files.
        self.raw_concepts = self._load_dim(concepts)
        self.raw_properties = self._load_dim(properties)
        self.raw_nominals = self._load_dim(nominals)

        # For RDF graphs, directly.

        self.concepts = [URIRef(c) for c in self.raw_concepts]
        self.properties = [URIRef(p) for p in self.raw_properties]
        self.nominals = [URIRef(n) for n in self.raw_nominals]

        # For property-graphs.

        self.values = [self.otherval] + self.nominals
