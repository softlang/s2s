...
Building Shapes2Shapes (S2S) requires only the Scala build tool [SBT](https://www.scala-sbt.org/), as well as Java.

0. Install the requirements.
1. Clone this repository.
2. Run an example (building automatically via SBT)
    * GNU/Linux or macOS: `./s2s tutorial/examples/q1.sparql tutorial/examples/S1.shacl`
    * Windows: `.\s2s.bat .\tutorial\examples\q1.sparql .\tutorial\examples\S1.shacl`
    * Any platform, from a `sbt` session: `runMain org.softlang.s2s.s2s tutorial/examples/q1.sparql tutorial/examples/S1.shacl`

Note, that running `s2s` from a SBT session is much faster for multiple executions, due to SBT startup time.

## Usage

Please also consider reading the full [tutorial](tutorial/README.md), which includes intuitive examples for the method itself.

The following examples assume usage of the `s2s` utility script; see the quick start section for system specific information, or for information on how to run the program from a SBT session, directly. Note: On Windows, you may want to set the active code page to UTF-8 via command [`chcp 65001`](https://learn.microsoft.com/en-us/windows-server/administration/windows-commands/chcp). The included `s2s.bat` script does so, automatically. This enables correct display of unicode symbols in output generated by `s2s`.
...
Alternatively, JSON-LD syntax can be used; the equivalent shape specification to the aforementioned axioms is given below. More examples are available in the [tutorial/examples](tutorial/examples/) folder. The syntax is determined by file extension, using `.json` for JSON-LD and `.shacl` for formal DL notation. Note, that JSON-LD support is still experimental.
