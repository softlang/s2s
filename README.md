...
Similarly, G-CORE and ProGS examples are available in the same folder.
Note that for ProGS we support only DL-like notation.

```sh
./s2s docs/ex/paper/q.gcore docs/ex/paper/S.progs
```
...
The provided tooling for validating both algorithm and implementation is documented in the respective [subproject](validation/README.md).
The remaining tools can be invoked via `sbt "runMain <MAIN-FN>"` and configured via their main methods; to this end, see [main](src/main/scala/org.softlang.s2s/main).
